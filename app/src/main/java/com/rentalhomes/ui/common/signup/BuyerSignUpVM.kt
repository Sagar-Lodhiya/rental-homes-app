...
import com.rentalhomes.data.pref.AppSessionManager.Companion.PREF_KEY_ACCESS_TOKEN
import com.rentalhomes.data.pref.AppSessionManager.Companion.PREF_KEY_EMAIL
import com.rentalhomes.data.pref.AppSessionManager.Companion.PREF_KEY_PROFILE_IMAGE
import com.rentalhomes.data.pref.AppSessionManager.Companion.PREF_KEY_USER_ID
import com.rentalhomes.data.pref.AppSessionManager.Companion.PREF_KEY_USER_TYPE
...
class BuyerSignUpVM(application: Application) :
    BaseViewModel<BuyerSignUpNavigator>(application) {
    ...
    var MLDFirebaseSignup: MutableLiveData<LoginRequest> = MutableLiveData<LoginRequest>()
    ...
    companion object {
        private val TAG = BuyerSignUpVM::class.simpleName
    }

    init {
        mldSignup.value = SignUpRequest(
            "", "", "", "", "", "",
            "", "", 0, DEVICE_TYPE_VALUE, deviceToken
        )
        deviceToken = "abcd123"
...
//                navigator.createAuthUserWithEmailAndPassword()
//                navigator.goToHome()
...
//                mResponse.message?.let { navigator.setMessageComingFromServer(it) }
    ...
    fun doTaskOnCreateUserWithEmailAndPasswordCompletion(task: Task<AuthResult>) {
        try {
            if (!task.isSuccessful) {
                navigator.hideProgress()
                AlertDialogUtils.showToast(context, "${task.exception?.localizedMessage}")
                Log.e(TAG, "user signup failed: ${task.exception?.localizedMessage}")
...
//                    addUser()
//                }
                Log.e(TAG, "user signup success: ${task.exception?.localizedMessage}")
            }
        } catch (e: Exception) {
            navigator.hideProgress()
...
//                    .show()
    ...
    private fun updateUserData(Id: String) {
        ...
        try {
            ...
            userDoc[Keys.DEVICE_TYPE] = DEVICE_TYPE_VALUE.toString()
            user.update(userDoc)
