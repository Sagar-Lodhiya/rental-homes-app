...
class FeedbackFeatureAdapter(
    private val context: Context,
    private var featuresList: ArrayList<RatingDataResponse.Tier3Data>,
) : RecyclerView.Adapter<FeedbackFeatureAdapter.MyViewHolder>() {
...
//        if (position == selected) {
        ...
        featuresList[position].categoryList?.let { categoryList ->
            ...
            val feedbackChildAdapter = FeedbackChildAdapter(context, categoryList)
            holder.binding.rvFeaturesChild.apply {
                this.adapter = feedbackChildAdapter
            }
        }
    }
